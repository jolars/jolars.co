...
configuration files, and workflows. When in doubt, refer to these instructions
first before searching through the codebase. Only search the codebase if:
...
The build process is straightforward: Quarto renders .qmd files to HTML, caching
computational results in `_freeze/`. The GitHub workflow deploys the rendered
site to GitHub Pages. There are no complex build steps, no test suite, and no
linting beyond what Quarto provides.
