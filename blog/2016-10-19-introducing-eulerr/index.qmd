...
area-proportional euler diagrams to display set relationships (intersections,
unions, and disjoints) with circles. [Euler
...
R features a number of packages that produce Euler and/or Venn diagrams; some of
the more prominent ones (on CRAN) are
...
venneuler, however, is written in java, preventing R users from browsing the
source code (unless they are also literate in java) or contributing.
Furthermore, venneuler is known to produce imperfect output for set
configurations that have perfect solutions. Consider, for instance, the
following example in which the intersection between `A` and `B` is unwanted.

```{r}
#| label: unwanted_overlap
...
```{r}
#| label: residual_plot
#| fig-cap: "Residuals for the eulerr fit."
#| fig-height: 4
#| fig-width: 4
#| fig-align: "center"
...
`fit2`. Given that these residuals are on the scale of the original values,
...
statistic that Leland Wilkinson presented in his academic paper on
venneuler\ [@wilkinson2012], which is given by the sums of squared residuals
divided by the total sums of squares:
...
input. Were it otherwise, we would do best to stop here and look for another way
to visualize our data. (I suggest the excellent
...
#| fig-height: 2.5
#| fig-show: "hold"
#| fig-cap: "eulerr plots can be modified in many ways."
#| fig-align: "center"
...
[qualpalr](https://cran.r-project.org/package=qualpalr) -- another package that
I have developed -- which uses color difference algorithms to generate distinct
